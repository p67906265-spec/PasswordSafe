...
class MainActivity : AppCompatActivity() {
    ...
    private fun showIncomingWindowsData(incoming:MutableList<VaultItem>){
        val ids=items.map{it.id}.toSet();val added=incoming.count{it.id !in ids};val updated=incoming.size-added
        AlertDialog.Builder(this).setTitle("Sincronizzazione incrementale").setMessage("Da Windows: $added voci nuove e $updated aggiornate. È stato trasferito soltanto questo gruppo di modifiche, non l’intera cassaforte.\n\nUnisci mantiene automaticamente la versione più recente di ogni voce.").setNegativeButton("Annulla",null).setPositiveButton("Unisci"){_,_->val merged=items.associateBy{it.id}.toMutableMap();incoming.forEach{remote->val local=merged[remote.id];if(local==null||remote.updatedAt>local.updatedAt)merged[remote.id]=remote};items.clear();items.addAll(merged.values);vault.save(items);toast("Aggiunte $added, aggiornate $updated");showCategoryMenu()}.show()
    }
